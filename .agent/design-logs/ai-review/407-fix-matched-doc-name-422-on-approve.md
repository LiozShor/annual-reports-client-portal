...
- [x] Confirmed CPA-XXX's "docs went pending" was NOT caused by this 422 — root cause is the inbound-dedup gap (DL-409).
...
- `api/src/lib/inbound/processor.ts` — dedup gap tracked separately (DL-409).
...
7. Found `processor.ts:549-557` comment confirming dedup is intentionally partial. Spun out as DL-409.
