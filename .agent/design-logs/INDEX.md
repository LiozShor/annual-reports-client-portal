...
| # | File | Status | Summary |
|---|------|--------|---------|
| 306 | [306-react-vite-ts-first-slice.md](admin-ui/306-react-vite-ts-first-slice.md) | DRAFT | **Learning + system task.** Introduce Vite + React 18 + TS strict + TanStack Query v5 + Vitest via a single island (client detail modal). Unlocks DL-132's "bundler prerequisite" for splitting script.js. Coexists as React island inside existing admin HTML; preserves `openClientDetailModalShared` API via bridge shim; reuses design-system CSS + `window.showConfirmDialog`. Built output committed to `frontend/admin/react-dist/` |
