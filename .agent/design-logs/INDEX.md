...
| # | File | Status | Summary |
|---|------|--------|---------|
| 293 | [293-wf02-extract-issuer-names.md](infrastructure/293-wf02-extract-issuer-names.md) | BEING IMPLEMENTED | WF02 calls new `/webhook/extract-issuer-names` (Haiku 4.5 + forced tool use) to extract issuer names from questionnaire free-text; stores as `issuer_name_suggested`; Review & Approve card shows bold ✨ chip with 1-click accept (+ "אשר הכל" batch); raw context preserved in `bookkeepers_notes` |
