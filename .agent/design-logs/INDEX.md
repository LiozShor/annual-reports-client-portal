...
| # | File | Status | Summary |
|---|------|--------|---------|
| 307 | [306-preuploaded-docs-indicator-pa-and-dm.md](admin-ui/306-preuploaded-docs-indicator-pa-and-dm.md) | IMPLEMENTED — NEED TESTING | PA card + doc-manager info banner when `pending_classifications` exist with `review_status='pending'`. Backend adds `pending_reviews_count` to `/admin-pending-approval` + `/get-client-reports`. Frontend: banner at top of `.pa-card__body` and below doc-manager `.page-header`; open-in-AI-Review button opens new tab with `?client=CPA-XXX` → AI Review scrolls + expands matching accordion (new `data-client-id` attr). Non-blocking; stage=`Pending_Approval` only. CSS uses `--info-*` tokens; duplicated into `document-manager.css` since doc-manager doesn't load admin CSS |
