...
**Last Updated:** 2026-04-28 (DL-372 + DL-373 — IMPLEMENTED, NEED TESTING; DL-372 adds PDF sticky-note annotation from AI Review overflow (📝 הוסף הערה ל-PDF), Worker endpoint `/webhook/add-pdf-note`, pdf-lib /Annot Text on page 1; DL-373 adds password-protected PDF unlock from preview panel — client-side PDF.js detection, Worker `/webhook/unlock-pdf` with `@localonlytools/pdf-decrypt`, archive-then-replace; both share new `MSGraphClient.putBinaryReplace`)
