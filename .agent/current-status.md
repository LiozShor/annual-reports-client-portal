...
**Last Updated:** 2026-05-03 (DL-399 — IMPLEMENTED, NEED TESTING. Email bounce handling shipped to main; live integration tests pending.)

## OPEN: DL-399 — Email bounce / NDR handling

DL: `.agent/design-logs/admin-ui/399-email-bounce-handling.md`

Open-test items from Section 7 (deploy Worker first; Pages auto-deploys on push):

- [ ] Send a real test email from `reports@moshe-atsits.co.il` to a known-bad address (e.g. `nonexistent-DL399@example.invalid`); within ~30s confirm an `email_bounce_handled` activity log entry, the bad client's `email` cleared, `last_bounced_email` populated, `email_bounce_reason` populated, `email_bounce_at` set.
- [ ] Stage-2 client receives a bounce → `reports.stage` flips to `Send_Questionnaire` AND `stage_reverted_on_bounce` activity log entry appears.
- [ ] Admin desktop: bounced client appears at top of clients table with red ⚠ button next to stage badge; click opens modal showing `last_bounced_email` + reason + formatted timestamp.
- [ ] Admin mobile: same red ⚠ button appears on the mobile card layout (Duplicate-Path Audit).
- [ ] Stage-1 stat card "ממתינים לשליחה" shows DL-187 needs-attention bounce animation when any stage-1 client has `email_bounced=true`.
- [ ] Green paper-plane row-action button is greyed/disabled with "אין כתובת מייל" tooltip when client `email` is empty.
- [ ] Bulk-send action: selecting a mix of empty-email and valid clients toasts `"דילגתי על N לקוחות ללא כתובת מייל"` and proceeds with the rest.
- [ ] Office edits a bounced client's email to a new value AND client is in Stage-1 → confirm dialog "לשלוח שאלון?" appears; on confirm, calls send-questionnaires endpoint.
- [ ] After email is fixed: ⚠ button disappears, client returns to normal sort position on next refresh, `email_bounced` flips to false, audit fields preserved.
- [ ] Idempotency: if the same NDR webhook fires twice (rare), no duplicate stage flip / no duplicate log entry.
- [ ] Regression: a real Hebrew out-of-office reply from a client is still classified as auto-reply (not bounce) — does NOT clear the email or revert stage.
- [ ] Hard-refresh — confirm `script.js?v=404` is served and `modules/bounce-warning.js?v=1` loads.

## Recent (last 7 days)

- **2026-05-03 · DL-399 — IMPLEMENTED, NEED TESTING.** Email bounce handling. Inbound `processor.ts` no longer silently swallows Outlook NDRs as auto-replies — new `bounce-detector.ts` (sonnet) + `bounce-handler.ts` (opus) parse subject/sender/body, extract failed recipient, classify hard vs soft, clear `clients.email`, write 4 audit fields, revert Stage-2 → Stage-1, log to activity-logger. Frontend (extracted to `modules/bounce-warning.js` due to size ratchet at zero headroom): clickable ⚠ button (desktop + mobile), pin bounced to top of table, Stage-1 stat-card needs-attention reuse of DL-187 keyframes, paper-plane row button + bulk-send disabled when `email` empty, post-edit "לשלוח שאלון?" confirm in Stage-1. Schema: 4 fields on `clients`, `Bounced` option on `email_events.processing_status`. 3/3 unit fixtures pass. DL: `.agent/design-logs/admin-ui/399-email-bounce-handling.md`.
