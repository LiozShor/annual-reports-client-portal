...
#### Scroll Behavior (DL-278)

When using **single-open accordions** (only one open at a time) inside a scrollable container:

1. **Delay scroll until collapse settles.** The closing accordion's `max-height` transition is 300ms (`--transition-slow`). Scroll to the new accordion *after* the layout has settled — use `setTimeout(..., 320)`.
2. **Add bottom padding** to the scroll container (`padding-bottom: 50vh`) so the last accordion can scroll to the top.
3. **Use `scrollIntoView({ behavior: 'smooth', block: 'start' })`** to smoothly bring the opened accordion to the top of its scroll container.

```js
// Single-open accordion with scroll-to behavior
function toggleAIAccordion(header) {
    ...
    if (!isOpen) {
        ...
        // Wait for collapse animation (300ms) to settle before scrolling
        setTimeout(() => accordion.scrollIntoView({ behavior: 'smooth', block: 'start' }), 320);
    }
}
```

**UX note:** Research (Smashing Magazine, Andrew Coyle) recommends *not* auto-closing other accordions to avoid layout shift. When single-open is required, the delay-then-scroll pattern above is the best compromise.
