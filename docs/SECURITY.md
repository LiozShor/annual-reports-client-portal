...
### ggshield
Calls the GitGuardian API with the staged diff. Uses ML-backed matching with awareness of AI-generated code patterns. Requires one-time `ggshield auth login` (free tier, browser OAuth). Credentials stored in `~/.gitguardian`.

`GITGUARDIAN_API_KEY` is set in `.env` — ggshield picks it up automatically when the env is sourced (`source .env`). The `ggshield auth login` browser flow is an alternative if the key is ever rotated or unavailable.
